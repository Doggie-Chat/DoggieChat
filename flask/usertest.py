...
#################################################################################################################################
    ...
    def test_6_dogs(self): # Test whether there are 6 dogs in the chat page if the login days are more than 7 days
        driver = self.driver
        self.driver.get("http://127.0.0.1:5000/chat")
        current_url = driver.current_url
        if "/login" in current_url:
        ...
        # Find all the dog tabs
        dog_tabs = self.driver.find_elements_by_css_selector(".dog-tab-parent .tab")

        # Assert that there are 6 dog tabs
        expected_dog_count = 6
        actual_dog_count = len(dog_tabs)
        self.assertEqual(actual_dog_count, expected_dog_count)
    ...
    def test_chat_with_dog(self):
        ...
        # Switch to Luna tab
        luna_tab = driver.find_element_by_id("tab_1")
        driver.execute_script("arguments[0].click();", luna_tab)

        # Click the "Chat with Luna" button
        chat_button = driver.find_element_by_id("Luna")
        driver.execute_script("arguments[0].click();", chat_button)

        # Simulate user input
        chat_input = driver.find_element_by_class_name("chat-input")
        ...
        ans_element = driver.find_element_by_xpath("(//div[@class='Ans'])[last()]").text
        original_content = "Woof woof! Hello there, I'm Luna. How can I assist you today?"
        self.assertIsNotNone(ans_element)
        self.assertNotEqual(ans_element, original_content)
...
# #################################################################################################################################
   # 4.Test the history page
    def test_history_search(self):
        ...
        date_input.clear()
        date_input.send_keys("16/05/2023")
    ...
    def test_search_without_date(self): # Test search functionality with only dog name and without date
        ...
        # Check if "Luna" are present in the table and "Jack" not in the table
        self.assertIn("Luna", table_text)
        self.assertNotIn("Jack", table_text)
        # Check if "2023-05-16","2023-05-15" are present in the table
    ...
    def test_search_without_dogname(self): # Test search functionality with only date and without dog name
        ...
        # Select "All" option
        dog_select = Select(self.driver.find_element_by_id("dogname"))
        dog_select.select_by_value("All")
        ...
        # Check if "Luna", "Jack" are present in the table
        self.assertIn("Luna", table_text)
        self.assertIn("Jack", table_text)

        # Check if "2023-05-16" are present in the table and "2023-05-15" not in the table
        self.assertNotIn("2023-05-15", table_text)
        self.assertIn("2023-05-16", table_text)
    ...
    def test_search_without_both(self): # Test search functionality without both date and dog name
        ...
        search_result_table = self.driver.find_element_by_css_selector("#search-result table")
        table_text = search_result_table.text
        # Check if "Luna", "Jack", "2023-05-16", "2023-05-15" are present in the table
        self.assertIn("Luna", table_text)
        self.assertIn("Jack", table_text)
        self.assertIn("2023-05-16", table_text)
        self.assertIn("2023-05-15", table_text)
